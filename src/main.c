...
/*   Updated: 2024/11/22 11:25:35 by anvacca          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_thread(t_gen *gen)
{
	...
	while (i < gen->philo_stats.nbr_of_philos)
	{
		...
		gen->philos[i].id = i + 1;
		gen->philos[i].death_timer = gen->philo_stats.time_start;
		gen->philos[i].meal = 0;
		pthread_create(&gen->philos[i].thread, NULL, &routine, &gen->philos[i]);
		i++;
	}
	if (gen->philo_stats.nbr_of_philos > 1)
		pthread_create(&gen->god, NULL, &death_god, &gen->philos);
